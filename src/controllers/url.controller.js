...
const CreateNewURLController = async (req, res) => {
  try {
    ...
    const CreateNewURLServiceResult = await CreateNewURLService(
      originalURL,
      keyId,
      userId
    );
    ...
    const {
      data: { keyId: keyIdFromDB },
    } = CreateNewURLServiceResult;
...
const RedirectURLController = async (req, res) => {
  try {
    ...
    const GetURLDetailsUsingItsKeyIdServiceResult =
      await GetURLDetailsUsingItsKeyIdService(keyId);

    if (!GetURLDetailsUsingItsKeyIdServiceResult.success) {
      const err = new Error(
        "Unable to fetch data from GetURLDetailsUsingItsKeyIdService"
      );
      err.statusCode = 400;
      throw err;
    }
    ...
    const {
      data: { _id: mongoId, originalUrl, clickedCount, createdAt },
    } = GetURLDetailsUsingItsKeyIdServiceResult;

    // check if clickedCount < 10
    if (clickedCount > 10) {
      const err = new Error(
        "You have reached the max limit of 10 request. Please upgrade for more limits"
      );
      err.statusCode = 400;
      throw err;
    }
    ...
    if (new Date().getTime() - createdAt > 7 * 24 * 60 * 60 * 1000) {
      const err = new Error(
        "Your redirect url is expired. Please upgrade for more expire limits"
      );
      err.statusCode = 400;
      throw err;
    }
    ...
    const UpdateTheURLUsingMongoIdServiceResult =
      await UpdateTheURLUsingMongoIdService(
        mongoId,
        geography?.region,
        geography?.country
      );
