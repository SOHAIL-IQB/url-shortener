...
const IsOrganizationPresentUsingOrgDomainService = async (
  organizationDomain
) => {
  try {
    ...
    if (organization) {
      ...
      throw new Error(
        `Unable to get organization details for domain : ${organizationDomain}`
      );
    }
  } catch (err) {
    console.log(
      `Error in isOrganizationPresentUsingOrgDomainService with err : ${err}`
    );
    return {
      success: false,
    };
...
const CreateNewOrganizationService = async (
  organizationDomain,
  organizationName
) => {
  try {
    ...
    if (organization) {
      ...
      throw new Error(
        `Unable to create organization for name : ${organizationName}`
      );
    }
  } catch (err) {
